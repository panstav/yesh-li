...
export default function Posts() {
	const { theme, content: { collectionPages: { post: posts, tag: availableTags } } } = useSiteData();

	const blogPostsPrefix = getCollectionPagePrefix(theme, 'post');
	const blogTagPrefix = getCollectionPagePrefix(theme, 'tag');
	...
	posts.forEach((post) => {
		...
		post.href = `/${blogPostsPrefix}/${post.slug}`;
	});

	// only show tags that have posts associated with them
	const usedTags = availableTags
		.filter(tag => posts.some(post => post.tags?.find((postTag) => postTag.title === tag.title)))
		.map((tag) => {
			tag.href = `/${blogTagPrefix}/${tag.slug}`;
			return tag;
		});
	...
	return <Section noTopMargin className="is-flex-desktop is-flex-direction-row-reverse is-align-items-start gap-6">
		<div className="is-flex gap-6">
			...
			<div className="is-flex-tablet-only gap-6" style={{ width: '100%', minWidth: '20ch' }}>
				...
				<div>
					...
					<ul className="tags">
						{usedTags.map(({ title, href }) => {
							return <li key={href} className="tag is-medium">
								<a href={href}>{title}</a>
							</li>;
						})}
					</ul>
		...
		<ol className="is-flex-grow-1">
			{posts.map(({ slug, title, href, publishDate, excerpt, featuredImage: { position, ...featuredImage } }) => {
				return <li key={slug} className={singlePostClassName}>
					<div className="is-flex-grow-1 reset-anchors">
						<a href={href} className="is-block has-text-black mb-2">
							<h2 className='is-size-4'>{title}</h2>
							<span className="is-size-7 has-text-weight-bold">{publishDate}</span>
							<div>{excerpt}</div>
						</a>
					</div>
					<a href={`/${blogPostsPrefix}/${slug}`} className="is-flex-shrink-0">
